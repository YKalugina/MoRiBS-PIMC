...
//Pot of H2O-C60
extern "C" void calengy_(double *com_1, double *E_H2OC60, double *Eulang_1);
#endif 
...
#ifdef MOLECULEINCAGE
  if ((IMPURITY && (MCAtom[IMTYPE].molecule == 2))||(MOLECINCAGE ))
#else
...
//         cout<<"calculate H2O in cage potential estim line 539 "<<endl; ///YULIAcout
      ...
      for (int it=0;it<NumbTimes;it++)
        {
         int t0 = offset0 + it;
             ...
             calengy_(com_1, &E_H2OC60, Eulang_1);
             spot_pair += E_H2OC60;
...
#ifdef MOLECULEINCAGE
        else if ( ((MCAtom[type0].molecule == 2) && (MCAtom[type1].molecule == 2)) && (MCAtom[IMTYPE].numb > 1) && ! MOLECINCAGE )
#else
...
//   cout<<"!!!!!!!!!!!!!GetPotEnergy_Densities!!!!!!!!estim lin 762 !pot= "<<spot/(double)NumbTimes<<endl;//YULIA
#endif
...
// double dr[NDIM];
   double spot = 0.0;
     ...
     //        cout<<"calculate H2O in cage potential GetPotEnergy line 770"<<endl;//YULIA
             double com_1[3];
             double Eulang_1[3];
             double E_H2OC60;
      #pragma omp parallel for reduction(+: spot_pair)
...
//      cout<<"estim line 973  spot= "<<spot<<endl;//YULIA
#endif
