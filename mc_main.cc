...
// chunksize = NumbRotTimes / numprocs;
   ...
   cout<<"MOLECINCAGE="<<MOLECINCAGE<<endl;
#endif
...
//YULIA                          IOxyzAng(IOWrite,fname.c_str());
                          PrintXYZprl = 0;
                       }
...
//                 cout<< "test YUL  "<<passCount<<" "<<passTotal<<" "<<totalCount<<" "<<sumsCount<<" "<<avergCount<<endl;
              }
           }  
...
//YULIA           SaveGraSum(MCFileName.c_str(),totalCount);
...
#ifdef MOLECULEINCAGE
//YULIA           if((IMPURITY && MCAtom[IMTYPE].molecule == 2)||MOLECINCAGE) 
#else
//YULIA           if((IMPURITY && MCAtom[IMTYPE].molecule == 2))
#endif
//YULIA           {
//YULIA              SaveDensities3D(MCFileName.c_str(),totalCount,MC_TOTAL); // this step takes lots of space.  temporarily turnned off
//YULIA              SaveRho1D(MCFileName.c_str(),totalCount,MC_TOTAL);
//            SaveRhoThetaChi(MCFileName.c_str(),totalCount,MC_TOTAL); // we don't need 2d angular distribution comparison now
//YULIA           }
//YULIA         if (ROTATION)                  // DUMP  accumulated average
//YULIA         SaveRCF(MCFileName.c_str(),totalCount,MC_TOTAL);
       }
...
#ifdef MOLECULEINCAGE
//  cout<<" skin line 570 main.cc !!!!"<< skin<<endl;//YULIA
//  cout<<" spot line 571 main.cc !!!!"<< spot<<endl;//YULIA
#endif
   double srot = 0.0;
...
//     double srot;
       ...
       srot = GetRotE3D();
#ifdef MOLECULEINCAGE
//       cout<<" srot line587 main.cc !!!!"<< srot<<endl;//YULIA 
#endif
      _brot       += srot;
      _rot_total  += srot;
...
// toby temporarily by-pass the density save for the non-linear rotor
   ...
   {
//YULIA   SaveDensities1D    (fname.c_str(),avergCount);

//YULIA   SaveRho1D          (fname.c_str(),avergCount,MC_BLOCK);

// SaveDensities3D    (fname.c_str(),avergCount,MC_BLOCK); // this step takes lots of space. temporarily turnned off

//YULIA   SaveRhoThetaChi    (fname.c_str(),avergCount,MC_BLOCK);
...
//YULIA      SaveRho1D          (fname.c_str(),avergCount,MC_BLOCK);
   ...
   if (ROTATION) 
//YULIA   SaveRCF            (fname.c_str(),avergCount,MC_BLOCK); 
